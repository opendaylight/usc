    ...
    public EchoServerUdp(final boolean enableEncryption) {
        b.group(bossGroup).channel(NioDatagramChannel.class).handler(new ChannelInitializer<DatagramChannel>() {
            ...
            public void initChannel(DatagramChannel ch) throws Exception {
                ...
                if(enableEncryption) {
                	p.addLast(new LoggingHandler("EchoServerUdp Handler 3", LogLevel.INFO));
                	p.addLast(secureService.getUdpServerHandler(ch));
                }
                p.addLast(new LoggingHandler("EchoServerUdp Handler 2", LogLevel.INFO));
                p.addLast(new EchoServerUdpHandler());
                p.addLast(new LoggingHandler("EchoServerUdp Handler 1", LogLevel.INFO));
            }
        });
