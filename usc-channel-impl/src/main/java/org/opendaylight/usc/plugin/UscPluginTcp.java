...
public class UscPluginTcp extends UscPlugin {
	...
	private final EventLoopGroup directGroup = new NioEventLoopGroup();
	private final Bootstrap agentBootstrap = new Bootstrap();
	private final Bootstrap directBootstrap = new Bootstrap();
	...
	protected Channel connectToAgent(UscDevice device) throws Exception {
		...
		try {
			channel = agentBootstrap.connect(device.getInetAddress(), agentPort).sync().channel();
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	protected Channel connectToDeviceDirectly(UscDevice device) throws Exception {
		Channel channel = null;

		try {
			channel = directBootstrap.connect(device.getInetAddress(), device.getPort()).sync().channel();
		} catch (Exception e) {
			throw e;
		}
