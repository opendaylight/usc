...
int options_cmdline ( char *name, char *parameter )
{
    ...
    else if ( !strcasecmp ( name, "-options" ) )
    {
        ...
        if ( !strcasecmp ( name, "-fd" ) )
        {
            if ( !parameter )
            {
                s_log ( LOG_ERR, "%s: No file descriptor specified",  __func__ );
                print_syntax();
                return 1;
            }
...
int options_parse ( CONF_TYPE type )
{
    ...
    s_log ( LOG_NOTICE, "%s: Reading configuration from %s %s", __func__,
    ...
    if ( type == CONF_FD ) /* file descriptor */
    {
        ...
        {
            s_log ( LOG_ERR, "%s: Invalid file descriptor number", __func__ );
            print_syntax();
            return 1;
        }
    ...
    if ( !df )
    {
        s_log ( LOG_ERR, "%s: Cannot open configuration file", __func__ );
    ...
    while ( file_getline ( df, line_text, CONFLINELEN ) >= 0 )
    {
        ...
        if ( line_number == 1 )
        {
            ...
            {
                s_log ( LOG_NOTICE, "%s: UTF-8 byte order mark detected", __func__ );
                config_opt += 3;
            }
            else
            {
                s_log ( LOG_NOTICE, "%s: UTF-8 byte order mark not detected", __func__ );
            }
        }
        ...
        if ( config_opt[0] == '[' && config_opt[strlen ( config_opt ) - 1] == ']' ) /* new section */
        {
            if ( !new_service_options.next )
            {
                ...
                if ( errstr )
                {
                    s_log ( LOG_ERR, "%s: Line %d: \"%s\": %s", __func__,
        ...
        if ( !config_arg )
        {
            s_log ( LOG_ERR, "%s: Line %d: \"%s\": No '=' found",  __func__, line_number, line_text );
            file_close ( df );
            return 1;
        }
        ...
        *config_arg++ = '\0'; /* split into option name and argument value */
        ...
        if ( errstr )
        {
            s_log ( LOG_ERR, "%s: Line %d: \"%s\": %s",  __func__, line_number, line_text, errstr );
            file_close ( df );
            return 1;
        }
    }
    ...
    if ( new_service_options.next ) /* daemon mode: initialize sections */
    {
        ...
        {
            s_log ( LOG_INFO, "%s: Initializing service [%s]",  __func__, section->servname );
    ...
    else     /* inetd mode: need to initialize global options */
    {
        ...
        if ( errstr )
        {
            s_log ( LOG_ERR, "%s: Global options: %s", __func__, errstr );
            return 1;
        }

        s_log ( LOG_INFO, "%s: Initializing inetd mode configuration", __func__ );
        section = &new_service_options;
        errstr = parse_service_option ( CMD_END, section, NULL, NULL );
    }
    ...
    if ( errstr )
    {
        s_log ( LOG_ERR, "%s: Service [%s]: %s",  __func__, section->servname, errstr );
        return 1;
    }

    s_log ( LOG_NOTICE, "%s: Configuration successful", __func__ );
    return 0;
}
...
NOEXPORT char *parse_global_option ( CMD cmd, char *opt, char *arg )
{
    ...
    {
        s_log ( LOG_NOTICE, "%s:  ",  __func__ );
        s_log ( LOG_NOTICE, "%s: Global options:", __func__ );
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = directory to chroot stunnel process", "chroot",  __func__ );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = compression type",
                "compression",  __func__ );
        break;
    }
    ...
    case CMD_DEFAULT:
#if !defined (USE_WIN32) && !defined (__vms)
        s_log ( LOG_NOTICE, "%s: %-22s = %s", "debug", "daemon.notice",  __func__ );
#else
        s_log ( LOG_NOTICE, "%s: %-22s = %s", "debug", "notice",  __func__ );
#endif
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = [facility].level (e.g. daemon.info)", "debug",  __func__ );
        break;
    }
    ...
    case CMD_DEFAULT:
#ifdef EGD_SOCKET
        s_log ( LOG_NOTICE, "%s: %-22s = %s", "EGD",  __func__, EGD_SOCKET );
#endif
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = path to Entropy Gathering Daemon socket", "EGD",  __func__ );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = auto|engine_id", __func__,
                "engine" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = cmd[:arg]",  __func__,
                "engineCtrl" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = TASK_LIST",  __func__,
                "engineDefault" );
        break;
    }
    ...
    case CMD_HELP:
#ifdef USE_FIPS
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no FIPS 140-2 mode",  __func__,
                "fips" );
#endif /* USE_FIPS */
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no foreground mode (don't fork, log to stderr)",  __func__,
                "foreground" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = icon when connections are established",  __func__, "iconActive" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = icon for invalid configuration file",  __func__, "iconError" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = icon when no connections were established",  __func__, "iconIdle" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = append|overwrite log file",  __func__,
                "log" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = file to append log messages",  __func__, "output" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = pid file",  __func__, "pid" );
        break;
    }
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = %d", "RNDbytes",  __func__, RANDOM_BYTES );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = bytes to read from random seed files",  __func__, "RNDbytes" );
        break;
    }
    ...
    case CMD_DEFAULT:
#ifdef RANDOM_FILE
        s_log ( LOG_NOTICE, "%s: %-22s = %s", "RNDfile",  __func__, RANDOM_FILE );
#endif
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = path to file with random seed data",  __func__, "RNDfile" );
        break;
    }
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = yes",  __func__, "RNDoverwrite" );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no overwrite seed datafiles with new random data",  __func__,
                "RNDoverwrite" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = service name",  __func__, "service" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = groupname for setgid()",  __func__, "setgid" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = username for setuid()",  __func__, "setuid" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = a|l|r:option=value[:value]",  __func__, "socket" );
        s_log ( LOG_NOTICE, "%s: %25sset an option on accept/local/remote socket",  __func__, "" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no send logging messages to syslog", __func__,
                "syslog" );
        break;
    }
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = yes",  __func__, "taskbar" );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no enable the taskbar icon",  __func__, "taskbar" );
        break;
    }
                                      ...
                                      char *opt, char *arg )
{
    ...
    {
        s_log ( LOG_NOTICE, "%s:  ",  __func__ );
        s_log ( LOG_NOTICE, "%s: Service-level options:",  __func__ );
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = [host:]port accept connections on specified host:port",  __func__,
                "accept" );
        break;
    }
    ...
    case CMD_DEFAULT:
#if 0
        s_log ( LOG_NOTICE, "%s: %-22s = %s", "CApath",
                section->ca_dir ? section->ca_dir : "(none)" );
#endif
        break;
    case CMD_HELP:
        s_log ( LOG_DEBUG, "%s: %-22s = CA certificate directory for 'verify' option",  __func__,
                "CApath" );
        break;
    }
    ...
    case CMD_DEFAULT:
#if 0
        s_log ( LOG_NOTICE, "%s: %-22s = %s", "CAfile",
                section->ca_file ? section->ca_file : "(none)" );
#endif
        break;
    case CMD_HELP:
        s_log ( LOG_DEBUG, "%s: %-22s = CA certificate file for 'verify' option",  __func__,
                "CAfile" );
        break;
    }
    ...
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = certificate chain",  __func__, "cert" );
        break;
    }
    ...
    case CMD_DEFAULT:
#ifdef USE_FIPS
        s_log ( LOG_NOTICE, "%s: %-22s = %s %s",  __func__, "ciphers",
                "FIPS", "(with \"fips = yes\")" );
        s_log ( LOG_NOTICE, "%s: %-22s = %s %s",  __func__, "ciphers",
                stunnel_cipher_list, "(with \"fips = no\")" );
#else
        s_log ( LOG_NOTICE, "%s: %-22s = %s", "ciphers",  __func__, stunnel_cipher_list );
#endif /* USE_FIPS */
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = list of permitted SSL ciphers",  __func__, "ciphers" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no client mode (remote service uses SSL)",  __func__,
                "client" );
        break;
    }
    ...
    case CMD_END:
        ...
        {
            s_log ( LOG_INFO,
                    "%s: ot resolve connect target - delaying DNS lookup",  __func__ );
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = [host:]port to connect",  __func__,
                "connect" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = CRL directory",  __func__, "CRLpath" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = CRL file",  __func__, "CRLfile" );
        break;
    }
...
#define DEFAULT_CURVE NID_X9_62_prime256v1
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = %s", "curve",  __func__, OBJ_nid2ln ( DEFAULT_CURVE ) );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = ECDH curve name", __func__, "curve" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no delay DNS lookup for 'connect' option",  __func__,
                "delay" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = ID of engine to read the key from",  __func__,
                "engineId" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = number of engine to read the key from", __func__,
                "engineNum" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = file execute local inetd-type program",  __func__,
                "exec" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = arguments for 'exec' (including $0)",  __func__,
                "execargs" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = rr|prio failover strategy",  __func__,
                "failover" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = username for IDENT (RFC 1413) checking",  __func__, "ident" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = certificate private key",  __func__, "key" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = OCSP server URL", __func__, "ocsp" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no check the AIA responders from certificates", __func__,
                "OCSPaia" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = OCSP server flags", __func__, "OCSPflag" );
        break;
    }
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = %s", __func__, "options", "NO_SSLv2" );
        s_log ( LOG_NOTICE, "%s: %-22s = %s",  __func__, "options", "NO_SSLv3" );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = SSL option", __func__, "options" );
        s_log ( LOG_NOTICE, "%s: %25sset an SSL option",  __func__, "" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = protocol to negotiate before SSL initialization",  __func__,
                "protocol" );
        s_log ( LOG_NOTICE, "%s: %25scurrently supported: cifs, connect, imap,",  __func__, "" );
        s_log ( LOG_NOTICE, "%s: %25s    nntp, pgsql, pop3, proxy, smtp",  __func__, "" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = authentication type for protocol negotiations",  __func__,
                "protocolAuthentication" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = host:port for protocol negotiations", __func__,
                "protocolHost" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = password for protocol negotiations", __func__,
                "protocolPassword" );
        break;
    }
    ...
    case CMD_END:
        ...
        else
        {
            if ( section->psk_identity )
                s_log ( LOG_NOTICE,
                        "PSK identity is ignored in the server mode",  __func__ );
        }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = identity for PSK authentication", __func__,
                "PSKidentity" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = secrets for PSK authentication",  __func__,
                "PSKsecrets" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no allocate pseudo terminal for 'exec' option",  __func__,
                "pty" );
        break;
    }
    ...
    case CMD_END:
        ...
        {
            s_log ( LOG_INFO,
                    "%s: Cannot resolve redirect target - delaying DNS lookup",  __func__ );
        }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = [host:]port to redirect on authentication failures",  __func__,
                "redirect" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no support renegotiation", __func__,
                "renegotiation" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no send TCP RST on error",  __func__,
                "retry" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = yes|no retry connect+exec section",  __func__,
                "retry" );
        break;
    }
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = %ld", "sessionCacheSize",  __func__, 1000L );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = session cache size",  __func__, "sessionCacheSize" );
        break;
    }
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = %ld seconds", "sessionCacheTimeout",  __func__, 300L );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = session cache timeout (in seconds)",  __func__,
                "sessionCacheTimeout" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = [host:]port use sessiond at host:port",  __func__,
                "sessiond" );
        break;
    }
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = master_service:host_name for an SNI virtual service",  __func__,
                "sni" );
        break;
    }
...
//                return "\"sslVersion = SSLv2\" not supported in FIPS mode";
    ...
    case CMD_DEFAULT:
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = all|SSLv2|SSLv3|TLSv1", __func__,
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = %d bytes", "stack",  __func__, DEFAULT_STACK_SIZE );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = thread stack size (in bytes)",  __func__, "stack" );
        break;
    }
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = %d seconds", "TIMEOUTbusy",  __func__, 300 );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = seconds to wait for expected data",  __func__, "TIMEOUTbusy" );
        break;
    }
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = %d seconds",  __func__, "TIMEOUTclose", 60 );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = seconds to wait for close_notify",  __func__,
                "TIMEOUTclose" );
        break;
    }
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = %d seconds",  __func__, "TIMEOUTconnect", 10 );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = seconds to connect remote host",  __func__, "TIMEOUTconnect" );
        break;
    }
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_NOTICE, "%s: %-22s = %d seconds",  __func__, "TIMEOUTidle", 43200 );
        break;
    case CMD_HELP:
        s_log ( LOG_NOTICE, "%s: %-22s = seconds to keep an idle connection",  __func__, "TIMEOUTidle" );
        break;
    }
    ...
    case CMD_FREE:
        break;
    case CMD_DEFAULT:
        s_log ( LOG_DEBUG, "%s: %-22s = none",  __func__, "verify" );
        break;
    case CMD_HELP:
        s_log ( LOG_DEBUG,
                "%s: %-22s = level of peer certificate verification",  __func__, "verify" );
        s_log ( LOG_DEBUG,
                "%s: %25slevel 0 - request and ignore peer cert",  __func__, "" );
        s_log ( LOG_DEBUG,
                "%s: %25slevel 1 - only validate peer cert if present",  __func__, "" );
        s_log ( LOG_DEBUG,
                "%s: %25slevel 2 - always require a valid peer cert",  __func__, "" );
        s_log ( LOG_DEBUG,
                "%s: %25slevel 3 - verify peer with locally installed cert",  __func__, "" );
        s_log ( LOG_DEBUG,
                "%s: %25slevel 4 - ignore CA chain and only verify peer cert",  __func__, "" );
        s_log ( LOG_DEBUG,
                "%s: %25slevel 5 - verify peer with CA chain, on fail check with local cert",  __func__, "" );
        break;
    }
...
NOEXPORT void print_ssl_options ( void )
{
    ...
    s_log ( LOG_NOTICE, "%s:  ",  __func__ );
    s_log ( LOG_NOTICE, "%s: Supported SSL options:",   __func__ );

    for ( option = ( SSL_OPTION * ) ssl_opts; option->name; ++option )
        s_log ( LOG_NOTICE, "%s: options = %s",  __func__, option->name );
}
...
NOEXPORT PSK_KEYS *psk_read ( char *key_file )
{
    ...
    if ( !df )
    {
        s_log ( LOG_ERR, "%s: Cannot open PSKsecrets file",  __func__ );
        return NULL;
    }
    ...
    while ( file_getline ( df, line, CONFLINELEN ) >= 0 )
    {
        ...
        {
            s_log ( LOG_ERR,
                    "%s: PSKsecrets line %d: Not in identity:key format",  __func__,
                    line_number );
            file_close ( df );
            psk_free ( head );
            return NULL;
        ...
        *key_val++ = '\0';
        ...
        {
            s_log ( LOG_ERR,
                    "%s: PSKsecrets line %d: Identity longer than %d characters",  __func__,
        ...
        {
            s_log ( LOG_ERR,
                    "%s: PSKsecrets line %d: Key longer than %d characters",  __func__,
        ...
        if ( key_len < 20 )
        {
            ...
            s_log ( LOG_ERR,
                    "%s: PSKsecrets line %d: Key shorter than 20 characters",  __func__,
                    line_number );
            file_close ( df );
            psk_free ( head );
            return NULL;
...
NOEXPORT int print_socket_options ( void )
{
    ...
    s_log ( LOG_NOTICE, "%s:  ",  __func__ );
    s_log ( LOG_NOTICE, "%s: Socket option defaults:",  __func__ );
    s_log ( LOG_NOTICE,
            "%s: Option Name     |  Accept  |   Local  |  Remote  |OS default",  __func__ );
    s_log ( LOG_NOTICE,
            "    ----------------+----------+----------+----------+----------" );
    ...
    for ( ptr = sock_opts; ptr->opt_str; ++ptr )
    {
                          ...
                          ptr->opt_name, ( void * ) &val, &optlen ) )
        {
            ...
            {
                s_log ( LOG_ERR, "%s: Failed to get %s OS default",  __func__, ptr->opt_str );
        ...
        s_log ( LOG_NOTICE, "%s:     %-16s|%10s|%10s|%10s|%10s",  __func__,
...
NOEXPORT char *engine_auto ( void )
{
    ...
    s_log ( LOG_DEBUG, "%s: Enabling automatic engine support",  __func__ );
    ...
    for ( e = ENGINE_get_first(); e; e = ENGINE_get_next ( e ) )
    {
        ...
        s_log ( LOG_INFO, "%s: Engine #%d (%s) registered",  __func__,
                current_engine + 1, ENGINE_get_id ( e ) );
    }

    engine_initialized = 1;
    s_log ( LOG_DEBUG, "%s: Automatic engine support enabled",  __func__ );
    return NULL; /* OK */
}
...
NOEXPORT char *engine_open ( const char *name )
{
    ...
    s_log ( LOG_DEBUG, "%s: Enabling support for engine \"%s\"",  __func__, name );
...
NOEXPORT char *engine_ctrl ( const char *cmd, const char *arg )
{
    ...
    if ( arg )
        s_log ( LOG_DEBUG, "%s: Executing engine control command %s:%s",  __func__, cmd, arg );
    else
        s_log ( LOG_DEBUG, "%s: Executing engine control command %s",  __func__, cmd );
...
NOEXPORT char *engine_default ( const char *list )
{
    ...
    s_log ( LOG_INFO, "%s: Engine #%d (%s) set as default for %s",  __func__,
...
NOEXPORT char *engine_init ( void )
{
    ...
    s_log ( LOG_DEBUG, "%s: Initializing engine #%d (%s)",  __func__,
    ...
    if ( !ENGINE_init ( engines[current_engine] ) )
    {
        if ( ERR_peek_last_error() ) /* really an error */
            ...
            s_log ( LOG_ERR, "%s: Engine #%d (%s) not initialized",  __func__,
                    current_engine + 1, ENGINE_get_id ( engines[current_engine] ) );
...
#endif
    s_log ( LOG_INFO, "%s: Engine #%d (%s) initialized",  __func__,
            current_engine + 1, ENGINE_get_id ( engines[current_engine] ) );
    engine_initialized = 1;
    return NULL; /* OK */
}
...
{
    s_log ( LOG_NOTICE, "%s:  ",  __func__ );
    s_log ( LOG_NOTICE, "%s: Syntax:",  __func__ );
    s_log ( LOG_NOTICE, "%s: stunnel ",  __func__,
#ifdef USE_WIN32
#ifndef _WIN32_WCE
            "[ [-install | -uninstall] "
#endif
    ...
    s_log ( LOG_NOTICE, "%s:     <filename>  - use specified config file",  __func__ );
#ifdef USE_WIN32
#ifndef _WIN32_WCE
    s_log ( LOG_NOTICE, "%s:     -install    - install NT service",  __func__ );
    s_log ( LOG_NOTICE, "%s:     -uninstall  - uninstall NT service",  __func__ );
#endif
    s_log ( LOG_NOTICE, "%s:     -quiet      - don't display a message box on success",  __func__ );
#else
    s_log ( LOG_NOTICE, "%s:     -fd <n>     - read the config file from a file descriptor",  __func__ );
#endif
    s_log ( LOG_NOTICE, "%s:     -help       - get config file help",  __func__ );
    s_log ( LOG_NOTICE, "%s:     -version    - display version and defaults",  __func__ );
    s_log ( LOG_NOTICE, "%s:     -sockets    - display default socket options",  __func__ );
}
