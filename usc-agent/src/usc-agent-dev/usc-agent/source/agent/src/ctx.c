...
int context_init ( SERVICE_OPTIONS *section ) /* init SSL context */
{
    ...
    if ( section->option.client && section->engine )
    {
        if ( SSL_CTX_set_client_cert_engine ( section->ctx, section->engine ) )
            s_log ( LOG_INFO, "%s: Client certificate engine (%s) enabled", __func__,
    ...
    s_log ( LOG_DEBUG, "%s: SSL options: 0x%08lX (+0x%08lX, -0x%08lX)", __func__,
            SSL_CTX_get_options ( section->ctx ),
            section->ssl_options_set, section->ssl_options_clear );
#else /* OpenSSL older than 0.9.8m */
    s_log ( LOG_DEBUG, "%s: SSL options: 0x%08lX (+0x%08lX)", __func__,
            SSL_CTX_get_options ( section->ctx ),
...
NOEXPORT int servername_cb ( SSL *ssl, int *ad, void *arg )
{
    ...
    {
        s_log ( LOG_DEBUG, "%s: SNI: no virtual services defined",  __func__ );
        return SSL_TLSEXT_ERR_OK;
    }

    if ( !servername )
    {
        s_log ( LOG_NOTICE, "%s: SNI: no servername received",  __func__ );
        return SSL_TLSEXT_ERR_NOACK;
    }

    s_log ( LOG_INFO, "%s: SNI: requested servername: %s",  __func__, servername );

    for ( list = section->servername_list_head; list; list = list->next )
        if ( matches_wildcard ( ( char * ) servername, list->servername ) )
        {
            s_log ( LOG_DEBUG, "%s: SNI: matched pattern: %s",  __func__, list->servername );
            ...
            s_log ( LOG_NOTICE, "%s: SNI: switched to service [%s]", __func__,
                    c->opt->servname );
#ifdef USE_LIBWRAP
    ...
    s_log ( LOG_ERR, "%s: SNI: no pattern matched servername: %s",  __func__, servername );
...
NOEXPORT int dh_init ( SERVICE_OPTIONS *section )
{
    ...
    s_log ( LOG_DEBUG, "%s: DH initialization", __func__ );
    ...
    if ( !dh )
    {
        s_log ( LOG_NOTICE, "%s: DH initialization failed", __func__ );
        return 1; /* FAILED */
    }

    SSL_CTX_set_tmp_dh ( section->ctx, dh );
    s_log ( LOG_DEBUG, "%s: DH initialized with %d-bit key",  __func__, 8 * DH_size ( dh ) );
    DH_free ( dh );
    return 0; /* OK */
}
...
NOEXPORT DH *read_dh ( char *cert )
{
    ...
    if ( !cert )
    {
        s_log ( LOG_DEBUG, "%s: No certificate available to load DH parameters", __func__ );
        return NULL; /* FAILED */
    }
    ...
    if ( !dh )
    {
        ...
        s_log ( LOG_DEBUG, "%s: Could not load DH parameters from %s",  __func__, cert );
        return NULL; /* FAILED */
    }

    s_log ( LOG_DEBUG, "%s: Using DH parameters from %s",  __func__, cert );
    return dh;
}
...
NOEXPORT DH *get_dh2048()
{
    ...
    s_log ( LOG_DEBUG, "%s: Using hardcoded DH parameters", __func__ );
    return dh;
}
...
NOEXPORT int ecdh_init ( SERVICE_OPTIONS *section )
{
    ...
    s_log ( LOG_DEBUG, "%s: ECDH initialization", __func__ );
    ecdh = EC_KEY_new_by_curve_name ( section->curve );

    if ( !ecdh )
    {
        sslerror ( "EC_KEY_new_by_curve_name" );
        s_log ( LOG_ERR, "%s: Cannot create curve %s", __func__,
    ...
    EC_KEY_free ( ecdh );
    s_log ( LOG_DEBUG, "%s: ECDH initialized with curve %s", __func__,
            OBJ_nid2ln ( section->curve ) );
    return 0; /* OK */
}
#endif /* OPENSSL_NO_ECDH */
                                        ...
                                        unsigned char *psk, unsigned max_psk_len )
{
    ...
    {
        s_log ( LOG_ERR, "%s: INTERNAL ERROR: No PSK identity selected", __func__ );
        return 0;
    }
    ...
    {
        s_log ( LOG_ERR, "%s: PSK identity too long (%lu>%d bytes)", __func__,
    ...
    {
        s_log ( LOG_ERR, "%s: PSK too long (%lu>%d bytes)", __func__,
    ...
    s_log ( LOG_INFO, "%s: PSK client configured for identity \"%s\"", __func__, identity );
                                        ...
                                        unsigned char *psk, unsigned max_psk_len )
{
    ...
    {
        s_log ( LOG_ERR, "%s: No key found for PSK identity \"%s\"",  __func__, identity );
        len = 0;
    }

    if ( len > max_psk_len )
    {
        s_log ( LOG_ERR, "%s: PSK too long (%lu>%d bytes)",  __func__, len, max_psk_len );
        len = 0;
    }
    ...
    if ( len )
    {
        ...
        s_log ( LOG_INFO, "%s: PSK server configured for identity \"%s\"",  __func__, identity );
    }
...
NOEXPORT int load_cert ( SERVICE_OPTIONS *section )
{
    ...
    if ( section->cert )
    {
        s_log ( LOG_INFO, "%s: Loading cert from file: %s",  __func__, section->cert );
    ...
    if ( !section->key )
    {
        s_log ( LOG_DEBUG, "%s: No private key specified", __func__ );
        return 0; /* OK */
    }
    ...
    s_log ( LOG_DEBUG, "%s: Private key check succeeded", __func__ );
    return 0; /* OK */
}
...
NOEXPORT int load_key_file ( SERVICE_OPTIONS *section )
{
    ...
    s_log ( LOG_INFO, "%s: Loading key from file: %s",  __func__, section->key );
    ...
    for ( i = 0; i <= 3; i++ )
    {
        ...
        if ( i <= 2 && reason == EVP_R_BAD_DECRYPT )
        {
            ...
            s_log ( LOG_ERR, "%s: Wrong pass phrase: retrying", __func__ );
            continue;
        }
...
NOEXPORT int load_key_engine ( SERVICE_OPTIONS *section )
{
    ...
    s_log ( LOG_INFO, "%s: Loading key from engine: %s", __func__, section->key );
    ...
    for ( i = 1; i <= 3; i++ )
    {
        ...
        if ( !pkey )
        {
            ...
            {
                sslerror_queue(); /* dump the error queue */
                s_log ( LOG_ERR, "%s: Wrong PIN: retrying", __func__ );
                continue;
            }
                               ...
                               unsigned char **ret, size_t *ret_len )
{
    ...
    s_log ( LOG_INFO,
            "cache_transfer: request=%s, timeout=%ld, id=%s, length=%ld", __func__,
            type_description[type], timeout, session_id_txt, val_len );

    /* allocate UDP packet buffer */
    if ( key_len > SSL_MAX_SSL_SESSION_ID_LENGTH )
    {
        s_log ( LOG_ERR, "%s: cache_transfer: session id too big (%ld bytes)", __func__,
                key_len );
        return;
    }
    ...
    {
        s_log ( LOG_ERR, "%s: cache_transfer: encoded session too big (%ld bytes)", __func__,
                key_len );
        return;
    }
    ...
    if ( len < 0 )
    {
        if ( get_last_socket_error() == S_EWOULDBLOCK ||
            ...
            s_log ( LOG_INFO, "%s: cache_transfer: recv timeout", __func__ );
        else
    ...
    {
        s_log ( LOG_DEBUG, "%s: cache_transfer: malformed packet received", __func__ );
        str_free ( packet );
        return;
    }
    ...
    {
        s_log ( LOG_INFO, "%s: cache_transfer: session not found", __func__ );
        str_free ( packet );
        return;
    }
    ...
    *ret = str_alloc ( *ret_len );
    s_log ( LOG_INFO, "%s: cache_transfer: session found", __func__ );
    ...
    SSL *ssl, int where, int ret )
{
    ...
    {
        s_log ( LOG_DEBUG, "%s: SSL state (%s): %s", __func__,
    ...
    {
        s_log ( LOG_DEBUG, "%s: SSL alert (%s): %s: %s", __func__,
    ...
    {
        s_log ( LOG_DEBUG, "%s: %4ld items in the session cache", __func__,
                SSL_CTX_sess_number ( ssl->ctx ) );
        s_log ( LOG_DEBUG, "%s: %4ld client connects (SSL_connect())", __func__,
                SSL_CTX_sess_connect ( ssl->ctx ) );
        s_log ( LOG_DEBUG, "%s: %4ld client connects that finished", __func__,
                SSL_CTX_sess_connect_good ( ssl->ctx ) );
        s_log ( LOG_DEBUG, "%s: %4ld client renegotiations requested", __func__,
                SSL_CTX_sess_connect_renegotiate ( ssl->ctx ) );
        s_log ( LOG_DEBUG, "%s: %4ld server connects (SSL_accept())", __func__,
                SSL_CTX_sess_accept ( ssl->ctx ) );
        s_log ( LOG_DEBUG, "%s: %4ld server connects that finished", __func__,
                SSL_CTX_sess_accept_good ( ssl->ctx ) );
        s_log ( LOG_DEBUG, "%s: %4ld server renegotiations requested", __func__,
                SSL_CTX_sess_accept_renegotiate ( ssl->ctx ) );
        s_log ( LOG_DEBUG, "%s: %4ld session cache hits", __func__,
                SSL_CTX_sess_hits ( ssl->ctx ) );
        s_log ( LOG_DEBUG, "%s: %4ld external session cache hits", __func__,
                SSL_CTX_sess_cb_hits ( ssl->ctx ) );
        s_log ( LOG_DEBUG, "%s: %4ld session cache misses", __func__,
                SSL_CTX_sess_misses ( ssl->ctx ) );
        s_log ( LOG_DEBUG, "%s: %4ld session cache timeouts", __func__,
...
void sslerror ( char *txt ) /* OpenSSL error handler */
{
    ...
    {
        s_log ( LOG_ERR, "%s: %s: Peer suddenly disconnected",  __func__, txt );
    }
}
...
NOEXPORT void sslerror_log ( unsigned long err, char *txt )
{
    ...
    s_log ( LOG_ERR, "%s: %s: %lX: %s", __func__, txt, err, error_string );
