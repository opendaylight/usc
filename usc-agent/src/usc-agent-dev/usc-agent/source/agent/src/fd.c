...
void get_limits ( void ) /* set max_fds and max_clients */
{
    ...
    if ( max_fds )
    {
        ...
        s_log ( LOG_DEBUG, "%s: Clients allowed=%ld",  __func__, max_clients );
    }
    else
    {
        max_clients = 0;
        s_log ( LOG_DEBUG, "%s: No limit detected for the number of clients", __func__ );
    }
}
...
NOEXPORT int setup_fd ( int fd, int nonblock, char *msg )
{
    ...
    {
        s_log ( LOG_ERR, "%s: %s: FD=%d out of range (max %ld)", __func__,
                msg, fd, max_fds );
        closesocket ( fd );
        return -1;
    }
    ...
    s_log ( LOG_DEBUG, "%s: %s: FD=%d allocated (%sblocking mode)", __func__,
            msg, fd, nonblock ? "non-" : "" );
...
void set_nonblock ( int fd, unsigned long nonblock )
{
    ...
    if ( ioctlsocket ( fd, FIONBIO, &nonblock ) < 0 )
        ...
        s_log ( LOG_DEBUG, "%s: Socket %d set to %s mode",
