...
void libwrap_auth ( CLI *c, char *accepted_address )
{
    ...
    {
        s_log ( LOG_INFO, "%s: Libwrap is not supported on Unix sockets", __func__ );
        return;
    }
    ...
    if ( num_processes )
    {
        s_log ( LOG_DEBUG, "%s: Waiting for a libwrap process", __func__ );
        ...
        s_log ( LOG_DEBUG, "%s: Acquired libwrap process #%d",  __func__, my_process );
        ...
        s_log ( LOG_DEBUG, "%s: Releasing libwrap process #%d", __func__, my_process );
        ...
        s_log ( LOG_DEBUG, "%s: Released libwrap process #%d",  __func__, my_process );
    }
    else
#endif /* USE_LIBWRAP_POOL */
    {
    ...
    if ( !result )
    {
        s_log ( LOG_WARNING, "%s: Service [%s] REFUSED by libwrap from %s", __func__,
                c->opt->servname, accepted_address );
        s_log ( LOG_DEBUG, "%s: See hosts_access(5) manual for details", __func__ );
        longjmp ( c->err, 1 );
    }

    s_log ( LOG_DEBUG, "%s: Service [%s] permitted by libwrap from %s", __func__,
            c->opt->servname, accepted_address );
}
...
NOEXPORT ssize_t read_fd ( int fd, void *ptr, size_t nbytes, int *recvfd )
{
    ...
    *recvfd = -1; /* descriptor was not passed */
    ...
    {
        s_log ( LOG_ERR, "%s: control level != SOL_SOCKET", __func__ );
        return -1;
    }

    if ( cmptr->cmsg_type != SCM_RIGHTS )
    {
        s_log ( LOG_ERR, "%s: control type != SCM_RIGHTS",  __func__, );
        return -1;
    }
