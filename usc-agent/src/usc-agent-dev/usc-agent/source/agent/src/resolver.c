                                  ...
                                  char *name, char *default_host )
{
    ...
    if ( *name == '/' )
    {
        ...
        {
            s_log ( LOG_ERR, "%s: Unix socket path is too long",  __func__ );
            return 0; /* no results */
        }
                             ...
                             char *host_name, char *port_name )
{
    ...
    for ( ;; )
    {
        ...
        s_log ( LOG_DEBUG, "%s: getaddrinfo: EAI_AGAIN received: retrying",  __func__ );
        sleep ( 1 );
    }
    ...
    case 0:
        break; /* success */
    case EAI_SERVICE:
        s_log ( LOG_ERR, "%s: Unknown TCP service \"%s\"",  __func__, port_name );
        return 0; /* error */
    default:
        s_log ( LOG_ERR, "%s: Error resolving \"%s\": %s",  __func__,
    ...
    for ( cur = res; cur; cur = cur->ai_next )
    {
        ...
        {
            s_log ( LOG_ERR, "%s: INTERNAL ERROR: ai_addrlen value too big",  __func__ );
            freeaddrinfo ( res );
            return 0; /* no results */
        }
...
#endif
    s_log ( LOG_ERR, "%s: INTERNAL ERROR: Unknown sa_family: %d",  __func__,
            addr->sa.sa_family );
