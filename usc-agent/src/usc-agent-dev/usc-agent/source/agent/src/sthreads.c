...
int sthreads_init ( void )
{
    ...
    if ( !new_context() )
    {
        s_log ( LOG_ERR, "%s: Cannot create the listening context",  __func__ );
        return 1;
    }
...
int create_client ( int ls, int s, CLI *arg, void * ( *cli ) ( void * ) )
{
    ...
    s_log ( LOG_DEBUG, "%s: Creating a new context",  __func__ );
    context = new_context();
    ...
    s_log ( LOG_DEBUG, "%s: New context created",  __func__ );
    return 0;
}
...
{
    ( void ) ls; /* this parameter is only used with USE_FORK */
    s_log ( LOG_DEBUG, "%s: Creating a new thread",  __func__ );
    ...
    s_log ( LOG_DEBUG, "%s: New thread created",  __func__ );
    return 0;
}
...
void stack_info ( int init ) /* 1-initialize, 0-display */
{
    ...
    else
    {
        ...
        if ( num < 64 )
        {
            s_log ( LOG_NOTICE, "%s: STACK_RESERVE is too high",  __func__ );
            return;
        }
        ...
        s_log ( LOG_NOTICE,
                "stack_info: size=%d, current=%d (%d%%), maximum=%d (%d%%)",  __func__,
                STACK_SIZE,
