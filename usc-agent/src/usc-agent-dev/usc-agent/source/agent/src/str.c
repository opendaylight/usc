...
struct alloc_list_struct
{
 ...
 void set_alloc_tls ( ALLOC_TLS * );
 ALLOC_TLS *get_alloc_tls();
 ALLOC_LIST *get_alloc_list_ptr ( void *, const char *, int );
 void crypto_init();
 void str_free_function ( void * );
...
void str_init()
{
 ...
 void set_alloc_tls ( ALLOC_TLS *tls )
{
    if ( ready_head )
        ready_head->tls = tls;
...
void str_init()
{
 ...
 void set_alloc_tls ( ALLOC_TLS *alloc_tls )
{
...
void str_stats()
{
    ...
    if ( !alloc_tls )
    {
        s_log ( LOG_DEBUG, "%s: str_stats: alloc_tls not initialized",  __func__ );
        return;
    }
    ...
    s_log ( LOG_DEBUG, "%s: str_stats: %lu block(s), "
            "%lu data byte(s), %lu control byte(s)",  __func__,
    ...
    for ( alloc_list = alloc_tls->head; alloc_list; alloc_list = alloc_list->next )
    {
        ...
        s_log ( LOG_DEBUG, "%s: str_stats: %lu byte(s) at %s:%d",  __func__,
...
void str_free_debug ( void *ptr, const char *file, int line )
{
 ...
 ALLOC_LIST *get_alloc_list_ptr ( void *ptr, const char *file, int line )
{
    ALLOC_LIST *alloc_list;
...
#if OPENSSL_VERSION_NUMBER>=0x0090700fL
 void str_free_function ( void *ptr )
{
