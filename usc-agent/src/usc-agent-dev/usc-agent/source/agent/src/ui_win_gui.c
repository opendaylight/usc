...
NOEXPORT int gui_loop()
{
    ...
    s_log ( LOG_DEBUG, "%s: GUI message loop initialized",  __func__ );
    ...
    for ( ;; )
        ...
        case -1:
            ...
            return 0;
        case 0:
            s_log ( LOG_DEBUG, "%s: GUI message loop terminated",  __func__ );
            return msg.wParam;
        default:
                                        ...
                                        UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_LOG:
        break;
    default:
        s_log ( LOG_DEBUG, "%s: Window message: 0x%x(0x%hx,0x%lx)",
...
int pin_cb ( UI *ui, UI_STRING *uis )
{
    ...
    if ( !ui_data )
    {
        s_log ( LOG_ERR, "%s: INTERNAL ERROR: user data data pointer",  __func__ );
        return 0;
    }
...
NOEXPORT void invalid_config()
{
    ...
    win_log ( TEXT ( "" ) );
    s_log ( LOG_ERR, "%s: Server is down",  __func__ );
...
ICON_IMAGE load_icon_file ( const char *name )
{
    ...
    s_log ( LOG_ERR, "%s: Loading image from file not implemented on WCE",  __func__ );
    icon = NULL;
#endif
    str_free ( tname );
    return icon;
