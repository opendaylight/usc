...
NOEXPORT int create_pid ( void )
{
    ...
    {
        s_log ( LOG_DEBUG, "%s: No pid file being created",  __func__ );
        return 0;
    }
    ...
    if ( global_options.pidfile[0] != '/' )
    {
        ...
        s_log ( LOG_ERR, "%s: Pid file (%s) must be full path name",  __func__, global_options.pidfile );
        return 1;
    }
    ...
    if ( pf == -1 )
    {
        s_log ( LOG_ERR, "%s: Cannot create pid file %s",  __func__, global_options.pidfile );
        ioerror ( "create" );
        return 1;
    }
    ...
    {
        s_log ( LOG_ERR, "%s: Cannot write pid file %s",  __func__, global_options.pidfile );
        ioerror ( "write" );
        return 1;
    }
    ...
    str_free ( pid );
    close ( pf );
    s_log ( LOG_DEBUG, "%s: Created pid file %s",  __func__, global_options.pidfile );
    atexit ( delete_pid );
    return 0;
}
...
NOEXPORT void delete_pid ( void )
{
    ...
    s_log ( LOG_DEBUG, "%s: removing pid file %s",  __func__, global_options.pidfile );
